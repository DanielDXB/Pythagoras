...
(define (key n)
  (let ((e (first(rest(get-nth-quadruple n)))) (f (first(rest(rest(get-nth-quadruple n))))))
...
(define (make-it-integer L)
  (if (empty? L)
      '()
      (cond
        ((= (first L) 32) (cons (- (first L) 32) (make-it-integer (rest L))))
        (else (cons (- (first L) 96) (make-it-integer (rest L)))))))

(define (message->codes message)
  (make-it-integer (map char->integer (apply list(string->list message)))))
...
(define (make-it-string L)
  (if (empty? L)
      '()
      (cond
        ((= (first L) 0) (cons (+ (first L) 32) (make-it-string (rest L))))
        (else (cons (+ (first L) 96) (make-it-string (rest L)))))))

(define (codes->message codes)
  (list->string (map integer->char (make-it-string codes)))
  )
...
(define (extend-key key size)
  (let ((l (length key)))
        (if (< size l)
            (take key size)
            (append key (extend-key key (- size l))))))
...
(define (en/de-crypt function message key)
  (map (lambda (m k) (modulo (function m k) 27)) message (extend-key key (length message))))

(define (encrypt-codes message key)
  (en/de-crypt + message key))
